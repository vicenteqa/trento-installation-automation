...
NOTE: SSH keys are generated automatically during Terraform provisioning. You no longer need to provide your own SSH key paths.
...
If you're using Azure Pipelines (`azure-pipelines.yml`), configure these pipeline variables:
...
* `AZURE_VMS_LOCATION` - Azure region (e.g., `westeurope`)
* `SSH_USER` - SSH username for VMs (e.g., `azureadm`)

**Note:** SSH keys are generated automatically by Terraform, so you don't need to provide `SSH_PUBLIC_KEY_CONTENT` or `SSH_PRIVATE_KEY_PATH` in the pipeline variables.
...
SSH keys are automatically generated during Terraform provisioning:

* Terraform creates an ephemeral ED25519 SSH key pair
* The private key is saved to `.ssh-key-temp` (file permission: 0600)
* The public key is automatically configured on all VMs
* Scripts retrieve the key path from Terraform outputs
* Keys are excluded from git via `.gitignore`

**Benefits:**

* No need to manage SSH keys manually
* No risk of committing private keys to version control
* Each deployment gets fresh keys
* Keys can be safely deleted after testing

**Cleanup:**

To remove the generated SSH key after infrastructure cleanup:
...
[source,bash]
----
rm -rf certs .venv-ansible inventories logs .ssh-key-temp
----
